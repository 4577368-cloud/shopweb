...
| `src/app/[locale]/logistics/page.tsx` | ~785 | 编排 + 模板 CRUD；报价在 `use-logistics-quote-estimate` |
...
| **D · 拆页** | logistics Step 1–5：hooks + workflow body；报价/pipeline 外置 | tsc + 三步 UI 回归 | ✅ Step 5 本地 |
...
*下一动：logistics **批次 D 收尾**（模板 save/delete 编排、或停拆）；或 onboarding **批次 E**。*
