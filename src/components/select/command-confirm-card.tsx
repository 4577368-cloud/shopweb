...
export function CommandConfirmCard({
  ...
  const t = useT();
  const resolvedHeaderLabel = headerLabel ?? t("commandUi.confirmHeader");
  const resolvedConfirmLabel = confirmLabel ?? t("commandUi.confirmExecute");
  const resolvedExecutingLabel = executingLabel ?? t("commandUi.executing");
  const styles = THEME_STYLES[theme];

  return (
    <div className={cn("rounded-md border px-2.5 py-2", styles.card)}>
      <p className={cn("text-[10px] font-medium uppercase tracking-wide", styles.header)}>
        {resolvedHeaderLabel}
      </p>
      <h3 className={cn("mt-0.5 text-xs font-semibold", styles.title)}>
        {plan.operation}
      </h3>
      <p className={cn("mt-0.5 text-[10px]", styles.subtext)}>
        {t("commandUi.target", { label: plan.targetLabel })}
      </p>

      {loading ? (
        <p className={cn("mt-2 flex items-center gap-1.5 text-[11px]", styles.title)}>
          <Loader2 className="h-3.5 w-3.5 animate-spin" />
          {t("commandUi.generatingPreview")}
        </p>
      ) : error ? (
        ...
        <div className="mt-2 space-y-2">
          {preview.sections.map((section, si) => (
            <div key={si} className="space-y-1.5">
              {section.title ? (
                <p className={cn("text-[10px] font-medium", styles.subtext)}>
                  {section.title}
                </p>
              ) : null}
              {section.rows.map((row, ri) => (
                <div key={ri} className="space-y-1">
                  <div className="rounded border border-slate-200/80 bg-white/70 px-2 py-1.5">
                    ...
                    <p className={cn("mt-0.5 text-[11px] leading-relaxed", styles.before)}>
                      {row.before || t("commandUi.empty")}
                    </p>
                  </div>
                  <div className={cn("rounded px-2 py-1.5", styles.afterBg)}>
                    <p className={cn("text-[10px] font-medium", styles.header)}>
                      {t("commandUi.afterLabel", { label: row.label })}
                    </p>
                    <p className={cn("mt-0.5 text-[11px] font-medium leading-relaxed", styles.after)}>
                      {row.after}
                    </p>
                  </div>
                </div>
          ...
          {preview.extraNote ? (
            <p className={cn("text-[10px]", styles.subtext)}>{preview.extraNote}</p>
          ) : null}
        </div>
      ) : null}
      ...
      <div className="mt-2.5 flex flex-wrap gap-1.5">
        ...
        >
          ...
          {executing ? resolvedExecutingLabel : resolvedConfirmLabel}
        </Button>
        <Button
          size="sm"
          variant="secondary"
