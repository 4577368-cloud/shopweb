...
const CURRENT_PRODUCT_REF =
  /(?:这个|当前|该|此)\s*商品|这个\s*品|当前\s*品|当前商品|这个商品|(?:this|the|current|selected)\s+product\b/i;
...
/** Selected-row copy ops: 把它标题改成… / translate this product title… */
export function refersToCurrentProductForCopy(text: string): boolean {
  ...
  if (
    /\b(?:translate|rewrite|optimize)\b/i.test(text) &&
    /\b(?:this|the|current|selected)?\s*(?:product\s+)?(?:title|description|copy)\b/i.test(
      text
    )
  ) {
    return true;
  ...
  if (
    /\b(?:this|the|current|selected)\s+product\b/i.test(text) &&
    /\b(?:title|description|copy|translate)\b/i.test(text)
  ) {
    return true;
  }
  return false;
