 ...
 *
 * Same dual-track auth as billing/api.ts: standalone sends
 * `Authorization: Bearer <TANGBUY_TOKEN>`; embedded uses session Bearer.
 * Cookie-only fetch fails after platform login because `tb_access` is never set.
 */
